...
import { HandCoins, ReceiptIndianRupee, Home, User, HelpCircle, LogOut, Menu, X } from 'lucide-react';

const MENU_ITEMS = [
  { text: "Dashboard", path: "/", icon: <Home size={20} /> },
  { text: "Income", path: "/income", icon: <HandCoins size={20} /> },
  { text: "Expenses", path: "/expense", icon: <ReceiptIndianRupee size={20} /> },
...
const Sidebar = ({user, isCollapsed, setIsCollapsed}) => {
    ...
    //for overflow check in mobile
    useEffect(() => {
        document.body.style.overflow = mobileOpen ? "hidden" : "auto";
        return () => { document.body.style.overflow = "auto" };
    }, [mobileOpen]);

    // outside click collapse the sidebar
    useEffect(() => {
