...
const App = () => {
  ...
  //to update the user data in state and localStorage
  ...
  useEffect(() => {
	(async ()=> {
		try{
			...
			console.error("error in bootstrapping auth:",err);
		}finally{
			setIsLoading(false);
  ...
  useEffect(() => {
    try{
      ...
      console.error("error in saving transactions:", err);
    }
  },[transactions]);
